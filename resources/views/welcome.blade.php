            ...
            <div class="col-6 col-md-2">
                <a href="{{ route('categories.show', $category->slug) }}" class="text-decoration-none text-dark">
                    <div class="category-card h-100">
                        ...
                        @if($staticIcon)
                            <img src="{{ $staticIcon }}" class="img-fluid mb-2 d-block mx-auto" style="width: 64px; height: 64px; object-fit: contain;" alt="{{ $category->name }}">
                        @elseif($category->icon && (str_starts_with($category->icon, 'http') || str_starts_with($category->icon, '/')))
                            <img src="{{ $category->icon }}" class="img-fluid mb-2 d-block mx-auto" style="width: 64px; height: 64px; object-fit: contain;" alt="{{ $category->name }}">
                        @elseif($category->icon)
                             {{-- Assume emoji or font class --}}
                             <div class="display-4 mb-2">{{ $category->icon }}</div>
                        @else
                            <img src="/img/icon-services.png" class="img-fluid mb-2 d-block mx-auto" style="width: 64px; height: 64px; object-fit: contain;" alt="Category">
                        @endif
